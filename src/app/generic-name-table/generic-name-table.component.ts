...
export class GenericNameTableComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  createTable() {
    this.dialog.open(GenericNameTableCreateUpdateComponent).afterClosed().subscribe((table: Table) => {
      if (table) {
  ...
  updateTable(table) {
    this.genericNameTableService.findById(table.id).subscribe((currentTable: Table) => {
      const dialogRef = this.dialog.open(GenericNameTableCreateUpdateComponent, { data: currentTable });
